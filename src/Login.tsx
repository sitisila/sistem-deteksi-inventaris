...
interface LoginProps {
  ...
  t?: any;
  onLogin: (formData: any) => void | Promise<void>;
    ...
    if (isRegistering) {
      ...
      try {
        ...
        Swal.fire({
          ...
          text: lang === 'id' ? 'Email atau Password salah!' : 'Incorrect Email or Password!',
          icon: 'error',
          confirmButtonColor: '#5c1313', 
          customClass: {
            popup: 
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col md:flex-row font-sans no-print items-stretch">
        ...
        <div className="relative z-10 
          ...
          <p className="text-xl font-medium leading-relaxed text-white/80 italic pr-6 transition-all duration-300">
            {isRegistering
              ...
              (
                lang === 'id'
                  ? 'Daftar untuk akses aset laboratorium.'
                  : 'Register to access laboratory assets.'
              )
              : (
       
                  lang === 'id'
                    ? 'Pengelolaan, pemantauan dan peminjaman alat laboratorium.'
                    : 'Management, monitoring and borrowing of laboratory equipment.'
              )}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[#FDFDFD] relative overflow-y-auto">
        ...
        <div className="w-full max-w-md space-y-8 my-10 bg-white/80 backdrop-blur-sm p-8 
          rounded-3xl shadow-2xl shadow-black/5 border border-gray-100 relative z-10">
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-400 text-sm font-semibold leading-relaxed text-center md:text-left">
              {isRegistering
                ?
                (lang === 'id'
                    ? 'Buat akun baru Civitas Academica Telkom University Anda.'
                    : 'Create your new Telkom University Civitas Academica account.')
                : (lang === 'id'
                    ? 'Login menggunakan ' +
                      'akun Telkom University Anda.'
                    : 'Login using your Telkom University account.')}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-6 py-4 border-2 border-gray-100 rounded-2xl outline-none focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all duration-300 font-bold text-utama text-sm"
                    required
                  />
                ...
                {/* DINAMIS IDENTITAS & NOMOR HP */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* DINAMIS: NIM / NIP */}
                  <div className="space-y-1">
                    <label className="text-[10px] font-black 
                      text-gray-400 uppercase ml-1 tracking-widest">
                      NIM / NIP
                    </label>
                    <input
                      type="text"
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="user@telkomuniversity.ac.id"
                onChange={handleChange}
                className="w-full px-6 py-4 border-2 border-gray-100 rounded-2xl outline-none focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all duration-300 font-bold text-utama text-sm"
                required
              />
            ...
            <div className="flex justify-between px-1 pt-2">
              ...
              >
                {isRegistering
                  ?
                  (lang === 'id' ? 'Kembali Login' : 'Back to Login')
                  : (t?.register || (lang === 'id' ? 'Registrasi' : 'Register'))}
              </button>
            </div>
            ...
            <button
              ...
              type="submit"
              className="w-full py-4 bg-brand text-white rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-xl shadow-brand/20 hover:bg-utama transition-all duration-300 transform hover:-translate-y-0.5 active:scale-95 text-xs"
            >
              {isRegistering
                ?
                (lang === 'id' ? 'Daftar Sekarang' : 'Register Now')
                : (t?.loginBtn || (lang === 'id' ? 'Masuk' : 'Login'))}
            </button>
