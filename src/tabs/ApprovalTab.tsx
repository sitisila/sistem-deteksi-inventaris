...
const ApprovalTab: React.FC<ApprovalTabProps> = ({ t, loans, onApprove, onReject }) => {
  return (
              ...
              <tr key={loan.id} className="hover:bg-zinc-50/50 transition-colors group">
                ...
                <td className="p-6">
                  <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                    ...
                    {loan.status === 'PENDING' ? t.statusPending : 
                     loan.status === 'APPROVED' ? t.statusApproved : t.statusRejected}
                  </span>
                </td>
                  ...
                  {loan.status === 'PENDING' && (
                    <div className="flex justify-end gap-2">
                      <button 
                        ...
                        className="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all shadow-lg shadow-green-200 hover:scale-105 active:scale-95"
                      >
                        {t.approveBtn}
                      </button>
                      <button 
                        onClick={() => onReject(loan.id)}
                        className="bg-utama hover:bg-brand text-white px-5 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all shadow-lg shadow-zinc-200 hover:scale-105 active:scale-95"
                      >
                        {t.rejectBtn}
                      </button>
                    </div>
