...
const HomeTab: React.FC<HomeTabProps> = ({ t, assets, loans, setActiveTab, currentUser }) => {
  ...
  const handleEditGuideLink = async () => {
    ...
    if (url) {
      ...
      Swal.fire({
        title: 'Berhasil!',
        text: 'Tautan dokumen panduan berhasil diperbarui di perangkat ini.',
        icon: 'success',
        confirmButtonColor: '#5c1313',
        customClass: { popup: 'rounded-[2rem]' }
      });
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center gap-3 px-1">
            ...
            <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{t.logisticSummary || "Ringkasan Data Logistik"}</h4>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { label: t.totalAsset || "TOTAL ASET", val: assets?.length || 0, col: "text-utama", bg: "group-hover:bg-utama/5",
                icon: <svg className="w-5 h-5 text-utama" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg> },
              { label: t.borrowed || "DIPINJAM", val: assets?.filter(a => a.status === 'BORROWED').length || 0, col: "text-orange-500", bg: "group-hover:bg-orange-50",
              ...
              { label: t.loans || "PERMINTAAN", val: loans?.filter(l => l.status === LoanStatus.PENDING).length || 0, col: "text-brand", bg: "group-hover:bg-brand/5",
                icon: <svg className="w-5 h-5 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg> },
              { label: t.available || "TERSEDIA", val: assets?.filter(a => a.status === 'AVAILABLE').length || 0, col: "text-green-600", bg: "group-hover:bg-green-50",
              ...
              <div key={i} className="p-6 bg-white border border-gray-100/70 rounded-[1.5rem] shadow-md shadow-black/[0.02] hover:shadow-xl hover:shadow-black/[0.04] hover:-translate-y-1 transition-all duration-300 group flex items-center justify-between">
                ...
                <div className={`p-3 rounded-xl bg-gray-50 transition-colors duration-300 ${stat.bg}`}>{stat.icon}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-gradient-to-br from-zinc-900 to-utama text-white rounded-[1.5rem] shadow-md relative overflow-hidden group">
            {isAdmin && (
              <button onClick={handleEditGuideLink}
            ...
            <h5 className="text-xs font-black tracking-widest text-brand uppercase mb-1">{t.guideTitle || "Buku Panduan Lab"}</h5>
      ...
      <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-gray-100/80 shadow-sm">
        <div className="flex items-center justify-between mb-6 px-1">
          <h4 className="text-[10px] font-black text-utama uppercase tracking-[0.25em]">{t.recentActivity || "AKTIVITAS TERBARU"}</h4>
        ...
        <div className="space-y-3">
          {loans?.length > 0 ? loans.slice(0, 3).map((loan: any) => (
            <div key={loan.id} className="flex items-center justify-between p-4 bg-white rounded-2xl shadow-sm border border-gray-100/60 hover:border-brand/30 transition-all duration-200">
              ...
              <div className="text-[9px] font-bold text-gray-300 uppercase tracking-wider px-2">{t.timeAgo || "Baru saja"}</div>
            </div>
          )) : (
            <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200 flex flex-col items-center justify-center px-4">
              <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center mb-3 relative">
                ...
                <svg className="w-5 h-5 text-gray-400 relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5a2 2 0 012-2h2a2 2 0 002-2V7a2 2 0 012-2h4a2 2 0 012 2v3a2 2 0 002 2h2a2 2 0 012 2z" /></svg>
              </div>
              <h5 className="text-xs font-black text-utama uppercase tracking-wider mb-1">{t.noRecentData || "Belum Ada Aktivitas"}</h5>
