...
const LabsTab: React.FC<LabsTabProps> = ({ 
  ...
  // 🎯 FIX EROR: Deklarasi data labList lokal agar component LabsTab mengenali list nama lab kelompok lu woi!
      ...
      {!selectedLab ? (
        ...
        <div className="space-y-6">
          ...
          <button onClick={() => setSelectedLab(null)} className="flex items-center gap-2 text-gray-400 hover:text-brand font-black text-[10px] uppercase tracking-widest transition-colors border border-gray-100 bg-white px-5 py-2.5 rounded-xl active:scale-95">
            ...
            <span>{isEnglish ? 'BACK TO LIST' : 'KEMBALI KE LIST'}</span>
          </button>
          ...
          <div className="relative overflow-hidden bg-gradient-to-r from-brand to-utama py-6 px-10 rounded-[2rem] text-white shadow-sm">
            <div className="relative z-10">
              <h3 className="text-2xl lg:text-3xl font-black tracking-tight mb-1 uppercase leading-none">
                {labList.find(l => l.name === selectedLab || l.id === selectedLab)?.name || selectedLab}
              </h3>
          ...
          <div className="relative w-full max-w-2xl px-1">
            ...
            />
            <svg className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </div>

          {/* TOMBOL KLASIFIKASI KATEGORI */}
          <div className="flex gap-2 overflow-x-auto pb-3 pt-1 scrollbar-none snap-x px-1">
            {categories.map((cat) => {
              const isActive = selectedCategory === cat.id;
              const currentLangLabel = isEnglish ? cat.enLabel : cat.idLabel;
              return (
                <button
                  key={cat.id}
                  type="button"
                  onClick={() => setSelectedCategory(cat.id)}
                  className={`px-5 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-wider whitespace-nowrap transition-all snap-center border ${
                    isActive 
                      ? 'bg-utama text-white border-utama scale-100 shadow-sm' 
                      : 'bg-white border-gray-100 text-gray-400 hover:bg-slate-50'
                  }`}
                >
                  {currentLangLabel}
                </button>
            ...
            {filteredLabAssets.length > 0 ? (
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md font-black text-[9px] uppercase tracking-wider">
                          {isEnglish ? 'STOCK' : 'STOK'}: {realStock} PCS
                        </span>
              ...
              <div className="py-24 text-center bg-white rounded-[2.5rem] border border-gray-50 shadow-sm">
                <p className="text-gray-300 font-black uppercase tracking-[0.4em] text-xs">
                  {isEnglish ? 'NO ASSETS IN THIS CLASSIFICATION' : 'TIDAK ADA ASET DI KLASIFIKASI INI'}
                </p>
              </div>
            )}
          </div>
